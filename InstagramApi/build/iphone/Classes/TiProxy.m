...
//Rare exceptions to indicate a bug in the _instagramapi code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_instagramapi = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_instagramapi]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.instagramapi.module"
