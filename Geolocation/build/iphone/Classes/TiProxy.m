...
//Rare exceptions to indicate a bug in the _geolocation code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_geolocation = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_geolocation]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.geolocation.module"
